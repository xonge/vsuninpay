...
<tr>
    ...
    <td width="180">图片跳转地址</td><?php endif; ?>
    <td class="norightborder"></td>
</tr>
</tbody>
<?php if($items != ''): if(is_array($items)): $key = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($key % 2 );++$key;?><tbody class="item<?php echo ($ivo["id"]); ?>" name="div_add_del">
<tr>
    ...
    <?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?><td width="200">
      <input type="text" name="add[startpicurl][]" value="<?php echo ($ivo["startpicurl"]); ?>" class="px" id="startpicurl_<?php echo ($key); ?>" style="width:130px;">
<a href="###" onclick="upyunPicUpload('startpicurl_<?php echo ($key); ?>',700,400,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('startpicurl_<?php echo ($key); ?>')">预览</a>
    ...
    <td width="50" class="norightborder"><a href="javascript:void(0);" vid="<?php echo ($ivo["id"]); ?>" class="del">删除</a></td>
</tr>
...
<th>防恶意刷新：</th>
<td>
  <input type="radio" name="refresh" value="1" <?php if($vo["refresh"] == '1' or $vo["refresh"] == ''): ?>checked<?php endif; ?>>开启 &nbsp;&nbsp;
  <input type="radio" name="refresh" value="0" <?php if($vo["refresh"] == '0'): ?>checked<?php endif; ?>>关闭  &nbsp;(开启防刷新后一个ip地址只能投一票)
</td>
</tr>
<!--
<tr>
