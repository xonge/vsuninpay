...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<td>
		<?php if($list["status"] == 1): ?>已兑换
		<?php else: ?>
			未兑换<?php endif; ?>
	</td>
	<td class="norightborder">
		<a href="<?php echo U('Red_packet/prize_log',array('token'=>$token,'id'=>$list['packet_id'],'log_id'=>$list['log_id']));?>">查看红包</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Red_packet/change_del',array('token'=>$token,'id'=>$list['id'],'packet_id'=>$packet_id));?>');">删除</a>
	    
	    <?php if($list["type"] == 3 and $list["status"] == 0): ?>|		
			<a href="javascript:void(0);"  onclick="show_forms(<?php echo ($list["id"]); ?>)" style="color:#31a8e7;">充值确认</a><?php endif; ?>
    </td>
...
function show_forms(id){
    art.dialog.open('<?php echo U('Red_packet/show_forms',array('token'=>$token,'packet_id'=>$packet_id));?>&id='+id,{lock:false,title:'手机充值兑奖',width:500,height:300,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
</div>
</div>
