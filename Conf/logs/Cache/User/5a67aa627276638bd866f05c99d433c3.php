...
<link rel="stylesheet" href="/tpl/static/wall/css/screen_vote_free.css">
...
<div class="Panel VoteList" style="display: block; opacity: 1;">
    <div class="listTit">
        <span>项目列表</span>
        <span class="restart" onclick="javascript:window.location.reload();">刷新列表</span>
    </div>
    <ul>
        <?php if(is_array($vote_list)): $i = 0; $__LIST__ = $vote_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vote_list): $mod = ($i % 2 );++$i;?><li class="vl" lid="<?php echo ($vote_list["id"]); ?>">
                <p><?php echo ($vote_list["title"]); ?></p>
                <span class="record" onclick="showIntroDetail(<?php echo ($vote_list["id"]); ?>)">投票结果</span>
                <span class="begin <?php if($vote_list['is_end'] == 1): ?>disable<?php endif; ?>">进入投票</span>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
</div>

<div class="Panel Vote" style="opacity: 1; display: none;">
    <div class="title">
        <h2><?php echo ($vote_info["title"]); ?></h2> 
        <div class="inf">
            <span class="numbers"><em class="fz">0</em>/<em class="fm">0</em></span>
            <span class="inf-btn">开始投票</span>
        </div>  
    </div>
    <div class="panelBox more">
...
<script>
 var vote_id    = '';
 var scene_id   = <?php echo ($info["id"]); ?>;
 var vcount     = '';
</script>
<script src="/tpl/static/wall/js/scene_vote.js"></script>
<script>
function showIntroDetail(id){
    art.dialog.open('<?php echo U('Scene/show_vote',array('token'=>$token));?>&id='+id,{lock:false,title:'信息详情',width:1200,height:600,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
