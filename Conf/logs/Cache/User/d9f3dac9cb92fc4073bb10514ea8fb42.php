...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<td><?php echo ($list["prize_name"]); ?></td>
	<td><?php echo ($list["worth"]); ?></td>
	<!--<td><?php echo ($list["code"]); ?></td>-->
	<td>
		<?php if($list["is_reward"] == '2'): ?><font color="">已兑奖</font>
	...
	<!--
	<?php if($list["is_reward"] != '0'): ?><a href="javascript:void(0);"  onclick="show_forms(<?php echo ($list["id"]); ?>)" style="color:#31a8e7;">兑奖信息</a>
		|<?php endif; ?>
	-->
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Red_packet/log_del',array('token'=>$token,'id'=>$list['id'],'packet_id'=>$packet_id));?>');">删除</a>
    </td>
...
function show_forms(id){
    art.dialog.open('<?php echo U('Red_packet/show_forms',array('token'=>$token,'packet_id'=>$packet_id));?>&id='+id,{lock:false,title:'兑奖信息查看',width:500,height:300,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
</div>
</div>
