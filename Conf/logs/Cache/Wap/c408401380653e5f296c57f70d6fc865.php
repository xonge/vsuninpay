    ...
    <section class="pdlayout">
            ...
            <div class="activeRule-content  pdlayout pr">
				<div class="box">
					...
					<div class="Detail">
						<p id="prizetype">
							<h3  class="price">总金额：<i id="count"><?php echo ($reward_info["prize_name"]); ?></i></h3>
						</p>
						<p>
						...
						<!-- <p>
							兑奖码：
							<span id="prizetype" class="red"><?php echo ($reward_info["code"]); ?></span>
						</p> -->
						<!--
						<p>
							<input id="mobile" class="px" type="text" placeholder="请输入您的手机号" value="<?php echo ($fans['tel']); ?>" name="mobile">
							 <input id="sncode" class="px" type="text" placeholder="请输入您的兑奖码" value="" name="sncode"> 
							<input id="wxname" class="px" type="text" placeholder="请输入您的微信号" value="<?php echo ($fans['wechaname']); ?>" name="wxname">
							<input id="wechaid" type="hidden" value="<?php echo ($wecha_id); ?>" name="">
							<input id="id" type="hidden" value="<?php echo ($reward_info["id"]); ?>" name="">
							<input id="packet_id" type="hidden" value="<?php echo ($reward_info["packet_id"]); ?>" name="">
						</p>
						-->
						<p>
						...
						<script>
							$(function(){
								$('#btn').on('click',function(){
									...
									$.getJSON('./index.php?g=Wap&m=Red_packet&a=reward_sub', {token:'<?php echo ($token); ?>',wecha_id:'<?php echo ($wecha_id); ?>',log_id:"<?php echo ($reward_info["id"]); ?>",id:'<?php echo ($reward_info["packet_id"]); ?>',mobile:mobile,is_one:is_one,ptype:ptype,card:card,pwd:pwd}, function(data){
										alert(data.info);
										if(data.err == 0){
										setTimeout(function(){
											window.location.href="<?php echo U('Red_packet/my_packet',array('token'=>$token,'wecha_id'=>$wecha_id,'id'=>$reward_info['packet_id']));?>";
										}, 2000);
										} 	
									});  
								});
								...
								$('#is_one').on('change',function(){
									var is_one = $(this).val();
									$.getJSON('./index.php?g=Wap&m=Red_packet&a=get_sum', {token:'<?php echo ($token); ?>',wecha_id:'<?php echo ($wecha_id); ?>',log_id:"<?php echo ($reward_info["id"]); ?>",id:'<?php echo ($reward_info["packet_id"]); ?>',is_one:is_one}, function(data){
								...
								$('#ptype').on('change',function(){
									var ptype = $(this).val();
									if(ptype == 2){
										$.getJSON('./index.php?g=Wap&m=Red_packet&a=get_card', {token:'<?php echo ($token); ?>',wecha_id:'<?php echo ($wecha_id); ?>',id:'<?php echo ($reward_info["packet_id"]); ?>'}, function(data){
